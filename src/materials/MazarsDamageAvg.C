...
MazarsDamageAvg::MazarsDamageAvg(const InputParameters & parameters)
  : ScalarDamageBase(parameters),
    ...
    _damage_index_local(declareProperty<Real>(_base_name + "damage_index_local")),
    _damage_index_local_old(getMaterialPropertyOld<Real>(_base_name + "damage_index_local")),
    _average(this->template getUserObject<RadialAverage>("average").getAverage())
{
}

void
MazarsDamageAvg::initQpStatefulProperties()
{
...
void
MazarsDamageAvg::initialSetup()
{
  if (!hasGuaranteedMaterialProperty(_elasticity_tensor_name, Guarantee::ISOTROPIC))
    mooseError("MazarsDamageAvg requires that the elasticity tensor be guaranteed isotropic");
}

void
MazarsDamageAvg::updateQpDamageIndex()
{
  ...
  if (equiv_tensile_strain > kappa)
  {
    ...
    _damage_index_local[_qp] =
        std::max(alpha_t * tensile_damage + alpha_c * compressive_damage, 0.0);
  }
...
Real
MazarsDamageAvg::getDamageIndex()
{
  if (_qp == 0)
